...
router.post("/get-session-list", rpg.multiSQL({
    dbcon: pass.dbcon,
    sql: "select s.id, s.name, s.descr, s.status, s.type, (s.id in (select sesid from teams)) as grouped, (s.id in (select sesid from report_pair)) as paired from sessions as s, " +
