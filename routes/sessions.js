...
router.post("/get-session-list", rpg.multiSQL({
    dbcon: pass.dbcon,
    sql: "select distinct s.id, s.name, s.descr, s.status, s.type, (s.id in (select sesid from teams)) as grouped, (s.id in (select sesid from report_pair)) as paired, sr.stime from sessions as s left outer join status_record as sr on sr.sesid = s.id and s.status = sr.status, " +
