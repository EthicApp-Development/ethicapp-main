...
DB_CONNECTION_URI_TEST="postgresql://$DB_TEST_USER_NAME:$DB_TEST_USER_PASSWORD@localhost:5432/$DB_TEST_NAME"
...
envsubst '${DB_USER_NAME} ${DB_USER_PASSWORD} ${DB_NAME} ${DB_TEST_USER_NAME} ${DB_TEST_USER_PASSWORD} ${DB_TEST_NAME}' < /tmp/create-db.sql > ./create-db.sql
...
#Creating schema for test database
for sqlFile in ./schema/*.sql; do
    psql ${DB_CONNECTION_URI_TEST} --set ON_ERROR_STOP=1 --file ${sqlFile}
done
