...
The routes related to the content analysis are protected with the middleware `middleware.verifySession` in Ethicapp, to allow external connections to the routes the environment variable `CONTENT_ANALYSIS_API_KEY` value can be send using the HTML header `x-api-key`. The key is setup when the docker compose is built, and it can be change in the `.env` file.
#### 7.2 Docker Deploy
