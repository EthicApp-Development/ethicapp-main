...
## 7. Advance Options
### 7.1 Deploy Content Analysis Service

To extend the docker compose file with the service run the following:

```bash
docker compose -f docker-compose.yml -f docker-compose.content-analysis.yml up --build --detach
```
Service-related functions depend on the `CONTENT_ANALYSIS_SERVICE` environment variable, which can be enabled or disabled using `"True"` or `"False"`. As an option, the value can be assign in the command to always enable the service, using:
```bash
