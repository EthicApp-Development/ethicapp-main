...
app.controller("EthicssController", ["$scope", "$http", "$timeout", "$socket", "Notification", "$sce", "$uibModal", "ngIntroService", function ($scope, $http, $timeout, $socket, Notification, $sce, $uibModal, ngIntroService) {
    ...
    self.loadStageData = function () {
        $http.post("get-stages", {}).success(function (data) {
            self.stages = data;
            self.selectStage(self.stages.find(e => e.id == self.currentStageId).number-1, false);
            ...
            if (self.currentStage && self.currentStage.type == "team") {
                $http.post("get-team-stage", { stageid: self.currentStageId }).success(function (data) {
                    ...
                    if (self.currentStage.anon){
                        self.anonimName= alph[data[0]["progress"]]   
                    }
    ...
    self.selectDfSel=function(df,i){
        df.changeDF=true;
        df.select = i;
    ...
    var introOptionsEng= {
        steps: [{
            element: '#tabd0',
            intro: 'In this tab you will find the case to read.'
        }, {
            element: "#tabq0",
            intro: "In this tab you will find the list of items that you must categorize and justify."
        }, {
            element: "#seslistbtn",
            intro: "Using this button you can go back to the session list."
        }],
        showStepNumbers: false,
        showBullets: false,
        exitOnOverlayClick: true,
