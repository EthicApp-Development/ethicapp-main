...
app.controller("EthicssController", ["$scope", "$http", "$timeout", "$socket", "Notification", "$sce", "$uibModal", "ngIntroService", function ($scope, $http, $timeout, $socket, Notification, $sce, $uibModal, ngIntroService) {
    ...
    self.loadStageData = () => {
        ...
        if(self.currentStageId != null){
            $http.post("get-differentials-stage", {stageid: self.currentStageId}).success(data => {
                self.dfs = data;
                if(self.dfs.length > 0 && self.sel.length > 0){
                    self.populateDFs();
                }
            });
            $http.post("get-diff-selection-stage", {stageid: self.currentStageId}).success(data => {
                self.sel = data;
                if(self.dfs.length > 0 && self.sel.length > 0){
                    self.populateDFs();
                }
    ...
    self.selectStage = (i) => {
        ...
        $http.post("get-differentials-stage", {stageid: self.stages[self.selectedStage].id}).success(data => {
            self.dfsPrev = data;
            if(self.selPrev.length == self.dfsPrev.length && self.selPrev.length > 0){
                self.populateDFsPrev();
            }
        });
        $http.post("get-diff-selection-stage", {stageid: self.stages[self.selectedStage].id}).success(data => {
            self.selPrev = data;
