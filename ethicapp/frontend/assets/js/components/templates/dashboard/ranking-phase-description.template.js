const rankingPhaseDescriptionTemplate = `
    <div>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    <th>{{ 'actor_name_label' | translate }}</th>
                    <th>{{ 'justification_required_label' | translate }}</th>
                    <th>{{ 'min_words_label' | translate }}</th>
                </tr>
            </thead>
            <tbody>
                <tr ng-repeat="actor in $ctrl.phaseActors">
                    <td>{{ actor.name }}</td>
                    <td>{{ actor.justificationRequired ? ('yes_label' | translate) : ('no_label' | translate) }}</td>
                    <td>{{ actor.minJustificationLength }}</td>
                </tr>
            </tbody>
        </table>
    </div>`;
