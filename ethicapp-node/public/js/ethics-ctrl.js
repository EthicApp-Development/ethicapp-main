...
app.controller("EthicssController", ["$scope", "$http", "$timeout", "$socket", "Notification", "$sce", "$uibModal", "ngIntroService", function ($scope, $http, $timeout, $socket, Notification, $sce, $uibModal, ngIntroService) {
    var self = $scope;
    ...
    var updateChat = function updateChat(count) {
        $http.post("get-diff-chat-stage", { stageid: self.currentStageId }).success(function (data) {
            self.chatMsgs = {};
    ...
    self.loadStageData = function () {
        ...
        if (self.currentStageId != null) {
            $http.post("get-differentials-stage", { stageid: self.currentStageId }).success(function (data) {
                self.dfs = data;
                if (self.dfs.length > 0 && self.sel.length > 0) {
                    self.populateDFs();
                }
            });
            $http.post("get-diff-selection-stage", { stageid: self.currentStageId }).success(function (data) {
                self.sel = data;
                if (self.dfs.length > 0 && self.sel.length > 0) {
                    self.populateDFs();
                }
    ...
    self.selectStage = function (i) {
        ...
        $http.post("get-differentials-stage", { stageid: self.stages[self.selectedStage].id }).success(function (data) {
            self.dfsPrev = data;
            if (self.selPrev.length == self.dfsPrev.length && self.selPrev.length > 0) {
                self.populateDFsPrev();
            }
        });
        $http.post("get-diff-selection-stage", { stageid: self.stages[self.selectedStage].id }).success(function (data) {
            self.selPrev = data;
