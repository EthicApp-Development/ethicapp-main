...
app.controller("EditorController", [
    ...
    function ($scope, $http, $timeout, $socket, Notification) {
        ...
        self.selectText = () => {
            ...
            if (textDef.length < 2){
                Notification.warning(
                    "No ha seleccionado un texto para ser usado como una idea fuerza"
                );
                return;
            }
        ...
        self.getIdeas = () => {
            ...
            if(self.iteration > 1) {
                self.tabOptions = [
                    { name: "Individual", idx: 1 },
                    { name: "Reelaboración Anónima", idx: 0 }
                ];
                $http({
                    url: "get-team-ideas", method: "post", data: {iteration: 1}
                }).success((data) => {
                    self.ansIter1 = {};
                    data.forEach((ans) => {
                        ...
                        self.highlightSerial(
                            ans.serial, self.docIdx[ans.docid], self.secondaryApplier
                        );
                    });
                });
            }
            if(self.iteration > 2) {
                self.tabOptions = [
                    { name: "Individual", idx: 1 },
                    { name: "Reelaboración Anónima", idx: 2 },
                    { name: "Reelaboración Grupal", idx: 0 }
                ];
                $http({
                    url: "get-team-ideas", method: "post", data: {iteration: 2}
                }).success((data) => {
                    self.ansIter2 = {};
                    data.forEach((ans) => {
        ...
        self.copyIdea = (sel) => {
            console.log(sel);
            if (sel == null || (sel.copied != null && sel.copied) || 
                self.selections.length >= 3*self.documents.length) return;
            let textDef = {
                text:     sel.content,
        ...
        self.shared.sendReportIdeas = () => {
            if(self.shared.idReport != null){
                let postdata = {repid: self.shared.idReport};
                $http({
                    url: "clear-report-ideas", method: "post", data: postdata
                }).success((data) => {
                    if (data.status == "ok") {
